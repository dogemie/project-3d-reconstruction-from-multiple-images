...
# 🔹 경계선 강조 함수
def apply_outline(image, outline_thickness=1):
    """
    객체의 경계를 강조하는 함수.
    outline_thickness 값이 높을수록 경계선이 더 두꺼워짐.
    """
    edge_image = image.filter(ImageFilter.FIND_EDGES)  # 경계 검출 필터 적용
    edge_image = ImageOps.colorize(edge_image.convert("L"), black="black", white="white")  # 검은색 강조
    return Image.blend(image, edge_image, alpha=0.3)  # 원본 이미지와 블렌딩

# 배경 제거 함수 (정밀 제거 + 검은색 배경 적용 + 경계선 강조)
def remove_background(img_file):
    ...
    output_image = remove(
        ...
        alpha_matting_erode_size=2  # ✅ 경계선 얇게 유지
    )

    # 🔹 투명한 배경을 검은색으로 변환
    black_bg = Image.new("RGBA", output_image.size, (0, 0, 0, 255))  # 검은색 배경
    output_image = Image.alpha_composite(black_bg, output_image).convert("RGB")

    # 🔹 경계선 강조 적용
    output_image = apply_outline(output_image)

    output_path = bg_removed_dir / img_file.name
    output_image.save(output_path)
    print(f"🖼️ Background removed (black) and saved: {output_path}")
