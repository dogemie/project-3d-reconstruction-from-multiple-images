...
# 🔹 경계선 복구 함수
def restore_edges(image, kernel_size=3):
    """
    OpenCV를 사용하여 경계선을 복구하는 함수.
    kernel_size가 클수록 경계선이 더 강조됨.
    """
    np_image = np.array(image)
    gray = cv2.cvtColor(np_image, cv2.COLOR_RGB2GRAY)
    edges = cv2.Laplacian(gray, cv2.CV_8U, ksize=kernel_size)
    edges = cv2.dilate(edges, np.ones((3,3), np.uint8), iterations=1)  # 경계를 살짝 확장

    # 흰색 경계 추가
    np_image[edges > 20] = (255, 255, 255)  # 밝은 경계 추가 (배경보다 뚜렷하게)
...
def remove_background(img_file):
    ...
    output_image = remove(
        ...
        alpha_matting_erode_size=1  # ✅ 경계선 살리기 위해 줄임
    )
    ...
    # 🔹 경계선 복구 적용
    output_image = restore_edges(output_image, kernel_size=3)
